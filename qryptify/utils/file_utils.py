...
from qryptify.config.config import MonitoringConfig
...
def write_last_synced_time(file, last_synced_time):
    write_to_file(file, str(last_synced_time) + '\n')


def read_last_synced_file(file):
    with smart_open(file, 'r') as file_handle:
        return int(float(file_handle.read()))
...
def write_monitor_logs(stream_name, synced_block, chain_id):
    ...
    g = Gauge('last_block_synced',
              'Block synced', ['process', 'chain_id'],
              registry=registry)
...
def write_last_time_running_logs(stream_name, timestamp, threshold=3600):
    ...
    monitor_path = MonitoringConfig.MONITOR_ROOT_PATH
    registry = CollectorRegistry()
    g = Gauge('last_time_run',
              'Last time run', ['process', 'threshold'],
              registry=registry)
