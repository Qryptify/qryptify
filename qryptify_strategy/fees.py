...
# Simple TTL cache: symbol -> (maker_bps, taker_bps, expires_monotonic)
...
def _fetch_fee_bps_httpx(symbol: str, timeout: float) -> Tuple[float, float]:
    ...
    try:
        import httpx  # lazy import; ingestor already depends on httpx
    except Exception as e:  # pragma: no cover - fallback handled by caller
...
def _fetch_fee_bps_urllib(symbol: str, timeout: float) -> Tuple[float, float]:
    ...
    from urllib.request import urlopen  # nosec B310 - public API URL
...
def binance_futures_fee_bps(symbol: str,
    ...
    try:
        ...
        # Only cache reasonable values
        exp = now + max(ttl_seconds, 60.0)
        _CACHE[sym] = (maker, taker, exp)
        return maker, taker
    except Exception:
        # Do not cache failures to allow future retries
