...
class TimescaleRepo:
    ...
    # Exchange fees ----------------------------------------------------------
    def upsert_exchange_fees(self, rows: Iterable[dict]) -> int:
        """Insert or update fee snapshots. rows: {ts, symbol, maker_bps, taker_bps, source, note?}.
        Returns count affected (inserted + updated)."""
        conn = self._require_conn()
        batch = list(rows)
        if not batch:
            return 0
        sql = (
            "INSERT INTO exchange_fees (ts, symbol, maker_bps, taker_bps, source, note)\n"
            "VALUES (%(ts)s, %(symbol)s, %(maker_bps)s, %(taker_bps)s, %(source)s, %(note)s)\n"
            "ON CONFLICT (symbol, ts) DO UPDATE\n"
            "  SET maker_bps = EXCLUDED.maker_bps, taker_bps = EXCLUDED.taker_bps,\n"
            "      source = EXCLUDED.source, note = COALESCE(EXCLUDED.note, exchange_fees.note)"
        )
        try:
            with conn.cursor() as cur:
                cur.executemany(sql, batch)
    ...
    def fetch_fee_snapshots(
        self,
        symbol: str,
        start: Optional[datetime] = None,
        end: Optional[datetime] = None,
    ) -> list[dict]:
        """Fetch fee snapshots ordered by ts ASC for a symbol and optional range."""
        conn = self._require_conn()
        clauses = ["symbol=%s"]
        ...
        if end is not None:
            ...
            params.append(end)
        where = " AND ".join(clauses)
        sql = f"SELECT ts, symbol, maker_bps, taker_bps, source, note FROM exchange_fees WHERE {where} ORDER BY ts ASC"
        try:
            with conn.cursor() as cur:
                cur.execute(sql, params)
                rows = cur.fetchall() or []
            ...
            # If migration not applied yet, behave as empty
            return []

    def fetch_fee_at(self, symbol: str, ts: datetime) -> Optional[dict]:
        """Fetch the latest fee snapshot at or before ts for symbol."""
        conn = self._require_conn()
        sql = ("SELECT ts, symbol, maker_bps, taker_bps, source, note\n"
               "FROM exchange_fees\n"
